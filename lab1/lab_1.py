...
replace_na(df, "Fireplaces", value="N")
replace_na(df, "SaleCondition", value="Normal")
...
categorical_pipeline = Pipeline([('onehot',one_hot_encoder)])

numerical_pipeline = Pipeline([('imputer',median_imputer),
                               ('scaler', min_max_scaler)])

column_transformer = ColumnTransformer([
        ('categorical', categorical_pipeline, categorical_features.tolist()),
        ('numerical', numerical_pipeline, numerical_features.tolist())
...
def assess_regression_model(model, X_train, X_test, y_train, y_test) -> tuple[float, float]:
    ...
    train_predict_expm = np.expm1(train_predict)
    y_train_expm = np.expm1(y_train)

    test_predict_expm = np.expm1(test_predict)
    y_test_expm = np.expm1(y_test)

    # calculate train and test RMSE
    train_rmse = root_mean_squared_error(y_train_expm, train_predict_expm)
    test_rmse = root_mean_squared_error(y_test_expm, test_predict_expm)
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# // Możemy zauażyć że, rożnica między RMSE dla zbioru testowego i RMSE dla zbioru treningowego jest bardzo duża
# // Co może świadczyć o overfittingu i trzeba będzie zregularyzować nasz model
#
...
reg_ridge = Ridge(random_state=0)  # L2
reg_lasso = Lasso(random_state=0)  # L1
...
ridgeCV = RidgeCV(alphas=np.linspace(0.1,100,1000))
lassoCV = LassoCV(n_alphas=1000, random_state=0)
...
print(f"LassoCV alpha: {reg_lasso_alpha:.4f}")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Jak można zauważyć przy obu modelach widać lekkie oznaki overfittingu jednak nie aż takie jak poprzednio. Przy regularyzacji L1 widać znaczną poprawe w porównaniu co było do tego wcześniej
